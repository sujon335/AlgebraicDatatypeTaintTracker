...
fw=open('updated_version_app_wise_id_stats.csv','w+')
...
ff = open('updated_ver_AppsName.txt','r')
#infile=sys.argv[1]
#ff = open(infile,'r')
...
AndroidID2="Landroid/content/Content;.getContentResolver"
...
for file in Apps:
    ...
    IMEI_id_methods=find_methods(IMEI,"TopFreeapksOutputUpdated/"+file)
    IMSI_id_methods=find_methods(IMSI,"TopFreeapksOutputUpdated/"+file)
    android_id_methods=find_AndroidID_methods(AndroidID,"TopFreeapksOutputUpdated/"+file)
    android_id_methods =android_id_methods + find_AndroidID_methods(AndroidID2, "TopFreeapksOutputUpdated/" + file)
    mac_id_methods = find_methods(MacAddress, "TopFreeapksOutputUpdated/" + file)
    serial_id_methods = find_methods(SERIAL, "TopFreeapksOutputUpdated/" + file)
    ad_id_methods = find_methods(AdvertisingID, "TopFreeapksOutputUpdated/" + file)
    Guid_id_methods = find_methods(UUID, "TopFreeapksOutputUpdated/" + file)
    ...
    fw.write(str(file)  + ',' +str(im) +  ','+str(ims)+','+  str(aid) +','+  str(sid) + ','+str(mac)+ ',' + str(adid) + ',' + str(guid))
    fw.write('\n')
