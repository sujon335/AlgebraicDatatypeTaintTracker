...
def process_id_methods(idmethods,file,source):
    ...
    for idmetho in ims:
        ...
        if arr[0]=="`Ljava" or arr[0]=="`Landroid":
            continue
        search_method(idmetho, "TopFreeApps/" + file,source)
...
for file in Apps:
    ...
    for source in sources:
        if source=="AndroidID":
            id_methods[source]=find_AndroidID_methods(sources[source],"TopFreeApps/"+file)
        else:
            id_methods[source]=find_methods(sources[source],"TopFreeApps/"+file)
